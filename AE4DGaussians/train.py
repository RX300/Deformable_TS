...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname):
    # Print AEDeformer configuration at the start
    print("\n" + "="*60)
    print("🚀 4DGaussians AutoEncoder Configuration")
    print("="*60)
    print(f"📐 Network Architecture:")
    print(f"   • Network Width: {hyper.net_width}")
    print(f"   • Network Depth: {hyper.defor_depth}")
    print(f"   • Latent Dimension: {hyper.latent_dim}")
    print(f"   • Position Encoding Frequencies: {hyper.multires}")
    print(f"   • Blender Dataset Optimization: {hyper.is_blender}")
    
    print(f"\n🎯 Loss Configuration:")
    print(f"   • Latent Regularization Weight: {hyper.latent_reg_weight}")
    print(f"   • Time Smoothness Weight: {hyper.time_smoothness_weight}")
    print(f"   • Plane TV Weight: {hyper.plane_tv_weight}")
    
    print(f"\n🔧 Training Configuration:")
    print(f"   • Coarse Iterations: {opt.coarse_iterations}")
    print(f"   • Fine Iterations: {opt.iterations}")
    print(f"   • Deformation LR (Init→Final): {opt.deformation_lr_init:.6f} → {opt.deformation_lr_final:.6f}")
    
    print(f"\n🌍 Scene Configuration:")
    print(f"   • Scene Bounds: ±{hyper.bounds}")
    print(f"   • White Background: {dataset.white_background}")
    
    # Calculate estimated parameter count
    # AutoEncoder network size estimation
    pos_encoding_dim = 3 + 3 * 2 * hyper.multires  # 3 + 3*2*multires for position
    time_encoding_dim = 1 + 1 * 2 * (6 if hyper.is_blender else 10)  # time encoding
    input_dim = pos_encoding_dim + time_encoding_dim
    
    # Rough estimation of network parameters
    if hyper.is_blender:
        # Blender version has timenet
        estimated_params = (time_encoding_dim * 256 + 256 * 30) + \
                          (pos_encoding_dim + 30) * hyper.net_width + \
                          hyper.defor_depth * hyper.net_width * hyper.net_width + \
                          hyper.net_width * (3 + 4 + 3)  # output layers
    else:
        estimated_params = input_dim * hyper.net_width + \
                          hyper.defor_depth * hyper.net_width * hyper.net_width + \
                          hyper.net_width * (3 + 4 + 3)  # output layers
