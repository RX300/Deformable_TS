...
def deform_step_with_mode(deform, xyz, time_input, gaussians, use_autoencoder):
    """
    根据模式调用合适的deform.step方法
    """
    if use_autoencoder:
        return deform.step(
            xyz.detach(), 
        ...
        return deform.step(xyz.detach(), time_input)


def render_set(model_path, load2gpu_on_the_fly, is_6dof, name, iteration, views, gaussians, pipeline, background, deform, use_autoencoder=False):
    ...
    with torch.no_grad():
        ...
        if not skip_train:
            render_func(dataset.model_path, dataset.load2gpu_on_the_fly, dataset.is_6dof, "train", scene.loaded_iter,
                        ...
                        background, deform, dataset.use_autoencoder)
