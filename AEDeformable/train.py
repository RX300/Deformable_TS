...
def training(dataset, opt, pipe, testing_iterations, saving_iterations):
    ...
    deform = DeformModel(
        dataset.is_blender, 
        dataset.is_6dof, 
        use_autoencoder=dataset.use_autoencoder,
    ...
    for iteration in range(1, opt.iterations + 1):
        ...
        # 如果使用自编码器，添加正则化损失
        if dataset.use_autoencoder and iteration >= opt.warm_up:
            reg_loss = deform.deform.compute_regularization_loss(
                gaussians.get_xyz.detach(),
        ...
        with torch.no_grad():
            ...
            cur_psnr = training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
                                       ...
                                       dataset.load2gpu_on_the_fly, dataset.is_6dof, dataset.use_autoencoder)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene: Scene, renderFunc,
                    renderArgs, deform, load2gpu_on_the_fly, is_6dof=False, use_autoencoder=False):
    if tb_writer:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if use_autoencoder:
                        ...
                        d_xyz, d_rotation, d_scaling = deform.step(
                            xyz.detach(), 
                            time_input,
                            rotation=scene.gaussians.get_rotation.detach(),
                            scaling=scene.gaussians.get_scaling.detach()
                        )
                    else:
                        # 原始模式：只需要位置信息
...
if __name__ == "__main__":
    ...
    if args.use_autoencoder:
        print(f"Using AutoEncoder Deform Network with latent dim: {args.latent_dim}")
    else:
        print("Using Original Deform Network")
